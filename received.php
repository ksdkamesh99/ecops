...
<body oncontextmenu="return false;">
	...
	<div id="qqq">
			<h1 style="position: relative;left: 150px;font-family: 'Anton', sans-serif;top: 40px;"><center>Information Sharing-Received</center></h1>
			<?php 
			$id=$_SESSION['user'];
				$sql=" SELECT * FROM users WHERE username= '".$id."'";
				$result = mysqli_query($con,$sql);
			...
			$sql5="SELECT * FROM informationsharing WHERE (`tops` LIKE '%".$ps."%' ) AND (`todesig` LIKE '%".$desig."%' ) ";
			...
			$noo=1;
				if(mysqli_num_rows($result)>0){
					...
					while($results1=mysqli_fetch_array($result)){
						...
						echo "<div>message no:  ".$noo."<hr>"."SUBJECT:".$results1['subject']."<hr>FROM:".$results1['fromdesig']."<br>                                       FROM-PS:".$results1['fromps']."<hr>MESSAGE:<br>".$results1['text_']."<br>WITHREGARDS:<br>".$results1['regards']."</div>";
						$noo=$noo+1;
